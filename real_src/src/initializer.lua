...
local make_guidance_fn = require(f.."user_specified.make_guidance_fn")

local homing_loop_thread = require(f.."homing_loop")

local function begin_execution(GUIDANCE_ARGS, ACCELERATION, RADIUS, ANGLE_MODIF, PID_ARGS)
    ...
    local guidance_fn = make_guidance_fn(GUIDANCE_ARGS)

    parallel.waitForAll(
            time_obj.timer_thread,
            function() homing_loop_thread(ship_radar, ship_reader, guidance_fn, pursuer_controller, control_action) end
    )
end
