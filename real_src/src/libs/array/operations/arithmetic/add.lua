...
local function scalar_add(tbl, item)
...
local function add_array(tbl, other_tbl)
...
local function array_add(tbl, item)
    if type(item) == "number" then
        return scalar_add(tbl, item)
    elseif type(item) == "table" and item.is_array then
        return add_array(tbl, item)
    else
